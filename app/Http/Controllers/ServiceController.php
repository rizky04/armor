...
class ServiceController extends Controller
{
    ...
    {
        $query = Service::with(['vehicle.client', 'creator.pengguna', 'payments', 'payment'])->orderBy('id', 'desc');
    ...
    public function fastService(){
        ...
        $mechanics = Mechanic::all();
        $spareparts = Product::all();
        return view('services.createFastService', compact('vehicles', 'mechanics', 'spareparts'));
    }
        ...
        try {
            ...
            //paymentjikalunas
             ...
             ServicePayment::create([
                    ...
                    'reference'    => $service->nomor_service,
                    'note'         => $request->note,
                    'payment_date' => now(),
                    'created_by'   => Auth::id(),
                ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->status_pembayaran === 'hutang') {
            ...
            // hapus hutang jika bukan hutang
            Hutang::where('id_transaksi', $service->id_transaksi)->delete();
        }
        ...
        $service->update([
            ...
            'updated_by'   => Auth::id(),
            'total_cost'   => $grandTotal,
            'status_bayar' => $request->status_pembayaran ?? $service->status_bayar,
        ]);
        ...
        if ($request->status_pembayaran === 'lunas') {
            ...
            $service->payment()->updateOrCreate(
                ['service_id' => $service->id],
                [
            ...
            // Jika bukan lunas, hapus payment (jika ada)
