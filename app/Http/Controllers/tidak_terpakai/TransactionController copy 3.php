...
class TransactionController extends Controller
{
    ...
    {
        $columns = ['reference', 'customer_id', 'total', 'created_at'];
        ...
        return response()->json([
            ...
            'data' => $data->map(function ($trx) {
                return [
                    ...
                    'created_at' => $trx->created_at->format('Y-m-d H:i'),
                ];
            }),
        ]);
    }
...
//         $isFree = false;
//         $total  = $validated['total']; // default sesuai input
//         $message = 'Transaksi berhasil disimpan';

//         if ($promo) {
//             if ($customer->free_voucher > 0) {
//                 // Kalau ada voucher → transaksi gratis
//                 $isFree = true;
//                 $total = 0;
//                 $customer->free_voucher -= 1;
//                 $message = 'Transaksi berhasil disimpan. Anda menggunakan 1 voucher gratis. Sisa voucher: ' . $customer->free_voucher;
//             } else {
//                 // Tidak ada voucher → transaksi normal
//                 $customer->squence += 1;

//                 // Kalau sudah genap 7 kali → reset & kasih voucher
//                 if ($customer->squence >= $promo->buy_count) {
//                     $customer->squence = 0;
//                     $customer->free_voucher += $promo->free_count ?? 1; // default 1
//                     $message = "Transaksi berhasil disimpan. Anda mendapatkan voucher gratis!";
//                 }
//             }
//             $customer->save();
//         }
...
//             'tax'             => $validated['tax'],
//             'total_after_tax' =>  $isFree ? 0 : $validated['total_after_tax'],
//             'cash'            => $validated['cash'],
//             'change'          => $isFree ? $validated['cash'] : $validated['change'],
//         ]);
...
//             'message' => $message,
//             'is_free' => $isFree,
//             'data'    => $transaction->load(['items.product', 'customer'])
//         ]);
//     } catch (\Exception $e) {
//         DB::rollBack();
//         return response()->json([
...
public function store(Request $request)
{
    ...
    try {
        ...
        $totalNormal = 0; // total untuk non-cuci
        $totalCuci   = 0; // total untuk cuci
        $hasCuci     = false;
        $hasNonCuci  = false;

        // loop items → cek kategori
        foreach ($items as $item) {
        ...
        $grandTotal = $totalNormal + $totalCuci;

        // logika voucher & promo
        if ($promo) {
            if ($customer->free_voucher > 0 && $hasCuci) {
                // ada voucher & ada item cuci
                $isFree = true;
                $totalCuci = 0; // cuci gratis
                $grandTotal = $totalNormal; // bayar hanya non-cuci
                $customer->free_voucher -= 1;
                $message = 'Transaksi berhasil. 1 voucher digunakan. Sisa voucher: ' . $customer->free_voucher;
            } else {
                // tidak pakai voucher, tetap hitung sequence
                $customer->squence += 1;
                if ($customer->squence >= $promo->buy_count) {
                    $customer->squence = 0;
        ...
        $transaction = Transaction::create([
            ...
            'discount'        => $validated['discount'],
            'tax'             => $validated['tax'],
            'total_after_tax' => $isFree ? $grandTotal : $validated['total_after_tax'],
            'cash'            => $validated['cash'],
            'change'          => $isFree ? $validated['cash'] - $grandTotal : $validated['change'],
        ]);
