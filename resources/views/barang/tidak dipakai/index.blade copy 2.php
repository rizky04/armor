...
let searchQuery = '';
 ...
 window.userPermissions = {
                editBarang: @json(auth()->user()->can('product-edit')),
                deleteBarang: @json(auth()->user()->can('product-delete'))
            };
...
function loadBarang(page = 1, search = '') {
    $.get("{{ route('barang.data') }}", { page: page, search: search }, function(res) {
        let rows = '';
        ...
        res.data.forEach(b => {
            ...
            if (window.userPermissions.editBarang) {
                btnEdit =
                    `<button class="dropdown-item btn-edit" data-id="${b.id_barang}"><i class="fa-solid fa-pencil me-2"></i>edit</button>`;
            }
            if (window.userPermissions.deleteBarang) {
                btnDelete =
                    `<button class="dropdown-item btn-delete" data-id="${b.id_barang}"><i class="fa-solid fa-trash me-2"></i>delete</button>`;
            }
                    ...
                    <td>${b.hapus == 1 ? 'Ya' : 'Tidak'}</td>
                      <td class="text-center">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a type="button" class="dropdown-item btn-qr" data-id="${b.id_barang}"><i class="fa-solid fa-qrcode me-2"></i>Print</a>
                                                </li>
                                                <li>
                                                    ${btnEdit}
                                                </li>
