...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#due_date').closest('.col-lg-4').hide(); // sembunyikan dulu di awal

$('#status_bayar').on('change', function() {
    const status = $(this).val();
    const dueDateField = $('#due_date').closest('.col-lg-4');
    ...
    // Ambil data barang via jQuery AJAX
    $.ajax({
        url: "{{ route('select2.barangSemua') }}",
        type: "GET",
        dataType: "json",
        ...
        error: function (xhr, status, error) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal!',
                text: 'Tidak dapat memuat data barang.',
            });
            console.error("Error:", error);
        },
        complete: function () {
            // Tutup SweetAlert setelah selesai (baik sukses/gagal)
            Swal.close();
        }
    });
            ...
            <tr data-id="${item.id}">
                ...
                <td><button type="button" class="btn btn-sm btn-danger delete-row"><i class="fa-solid fa-trash"></i>Hapus</button></td>
            </tr>
        `;
            ...
            $.ajax({
                ...
                success: res => {
                        Swal.fire('Berhasil', 'Transaksi berhasil disimpan!', 'success').then(() => {
                            ...
                            $('#btnSimpan').prop('disabled', true).text('Tersimpan');

                            // 🖨 tambahkan tombol print di sebelah kanan
                            if ($('#btnPrint').length === 0) {
                                $('#btnSimpan').after(`
                                    <button type="button" id="btnPrint" class="btn btn-info ms-2">
                            ...
                            $('#btnPrint').on('click', function() {
                                window.open(`/sales/${res.data.id}/print`, '_blank');

                                // reset form setelah print amount_paid
                                setTimeout(() => {
                                    $('#barangTable tbody').empty();
                                    $('#grandTotal').text('Rp 0');
                                    $('#service-form')[0]?.reset();
                                    $('#id_client').val(null).trigger('change');
                                    $('#amount_paid').val('');
                                    $('#note').val('');
