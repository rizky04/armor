...
<div class="page-header">
    <div class="page-title">
        ...
        <h6>Input Penjualan Platform</h6>
    </div>
    <div class="page-btn">
    ...
    <div class="card-body">
            ...
            <div class="col-md-2">
                ...
                <select id="platform" class="form-select">
                    @foreach($platformList as $p)
                        <option value="{{ $p }}" {{ $p === 'Shopee' ? 'selected' : '' }}>{{ $p }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-3">
                <label class="form-label">No. Pesanan Platform</label>
                <input type="text" id="nomor_pesanan" class="form-control" placeholder="Contoh: 250514XXXX">
            </div>
            <div class="col-md-3">
                <label class="form-label">Nama Pembeli</label>
                <input type="text" id="nama_pembeli" class="form-control" placeholder="Nama pembeli">
            </div>
            <div class="col-md-2">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                ...
                <input type="number" id="biaya_admin" class="form-control" value="0" min="0"
                       placeholder="Misal: biaya komisi Shopee">
                <small class="text-muted">Bisa hitung dari % komisi × total jual</small>
            </div>
            <div class="col-md-4">
                <label class="form-label">Biaya Pengiriman (Rp)</label>
                <input type="number" id="biaya_pengiriman" class="form-control" value="0" min="0"
                       placeholder="Ongkir yg dipotong dari penghasilan">
            </div>
            <div class="col-md-4">
                <label class="form-label">Biaya Lainnya (Rp)</label>
                <input type="number" id="biaya_lainnya" class="form-control" value="0" min="0"
                       placeholder="Biaya penyesuaian, retur, dll">
            </div>
        </div>

        <!-- RINGKASAN SEPERTI SHOPEE -->
        ...
        <div class="d-flex justify-content-end">
            <button class="btn btn-success" id="btnSimpan">
                <i class="fa-solid fa-floppy-disk me-1"></i> Simpan Penjualan
            </button>
        </div>
...
$(document).ready(function() {
    ...
    function tambahBarang(item) {
        ...
        // Harga jual default = harga jual master, tapi bisa diubah
        ...
        const tr = $(`
            <tr data-id="${item.id_barang}" data-kode="${item.kode_barang ?? ''}"
                ...
                <td class="text-end text-muted" id="kulak-${item.id_barang}">${fmt(hargaKulak)}</td>
                <td>
                ...
                <td>
                    <input type="number" class="form-control form-control-sm qty text-center"
                           value="1" min="1" max="${item.stok}" style="width:70px">
                </td>
    ...
    $('#btnSimpan').on('click', function() {
        ...
        Swal.fire({
            ...
            $.ajax({
                url: "{{ route('penjualan-platform.store') }}",
                type: 'POST',
                data: {
                ...
                success: function(res) {
                    Swal.fire('Berhasil!', res.message, 'success').then(() => {
                        ...
                        if ($('#btnPrint').length === 0) {
                            $('.d-flex.justify-content-end').prepend(`
                                <a href="/penjualan-platform/${res.data.id}/print" target="_blank"
                                   class="btn btn-info me-2" id="btnPrint">
