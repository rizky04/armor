...
//                             if (res.status) {
//                                 // update select dengan jasa baru
//                                 const newOption = new Option(res.data.nama_jasa, res.data.id_jasa, true, true);
//                                 el.append(newOption).trigger('change');

//                                 row.find('.job-price').val(formatRupiah(res.data.harga_jasa)).data('raw', res.data.harga_jasa);
//                                 row.find('.job-price-hidden').val(res.data.harga_jasa);
//                                 row.find('.job-qty').val(1);
//                                 calculateGrandTotal();

//                                 Swal.fire('Berhasil', 'Jasa baru ditambahkan', 'success');
//                             }
//                         },
//                         error: () => Swal.fire('Gagal', 'Tidak bisa menyimpan jasa baru', 'error')
//                     });
//                 }
//             });
//         } else {
//             // Jika jasa dipilih dari master
//             row.find('.job-price').val(formatRupiah(data.price)).data('raw', data.price);
//             row.find('.job-price-hidden').val(data.price);
//             row.find('.job-qty').val(1);
//             calculateGrandTotal();
...
function initJobSelect(el) {
    ...
    .on('select2:close', function() {
        ...
        if (val && typeof val === 'string' && val.startsWith('new:')) {
            let data = { id: val, text: val.replace('new:', '') + ' (buat baru)', newOption: true };
        ...
        el.select2({
            ...
            ajax: {
                ...
                processResults: data => ({
                    results: data.map(item => ({
                        id: item.id_barang,
                        text: item.id_barang + ' - ' + item.nama_barang + ' - ' + item.kode_barang + ' - ' + item.jenis + ' - ' + item.merk_barang,
