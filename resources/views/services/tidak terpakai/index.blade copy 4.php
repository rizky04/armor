    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status) {
                ...
                // ==== Isi otomatis kolom "Dibayar" ====
                $('#input-bayar').val(sisa.toLocaleString('id-ID'));

                // Hitung kembalian awal (biasanya 0 karena jumlah sama)
                $('#kembalian').val('Rp 0');

                // Fokuskan ke input bayar (biar bisa langsung edit manual)
                $('#input-bayar').focus();
            }
        },
        error: function () {
...
// ===================================================
// 2️⃣ FORMAT OTOMATIS INPUT BAYAR + HITUNG KEMBALIAN
// ===================================================
...
$(document).on('submit', '#form-create-payment', function (e) {
    ...
    // Ambil nilai bayar tanpa titik
    const rawBayar = $('#input-bayar').val().replace(/[^\d]/g, '');
    const formData = $(this).serialize() + `&bayar=${rawBayar}`;
