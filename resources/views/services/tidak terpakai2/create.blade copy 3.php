                ...
                <div class="row mt-3">
                    ...
                    <div class="col-lg-12 col-sm-12 col-12">
                        <div class="form-group">
                            ...
                            <textarea name="note" id="note" class="form-control" rows="2" placeholder="Catatan tambahan..."></textarea>
...
$(document).on('input', '#amount_paid_display', function() {
    updatePaymentFields();
        ...
        //                             // });
        //                              // ✅ Tambahan tombol print setelah simpan jika lunas
        //               // ✅ Jika status pembayaran lunas → tampilkan tombol print di Swal
        //               console.log('Response Data:', res.data); // Debug: cek data response
        //               console.log('Status Pembayaran:', res.data ? res.data.status_pembayaran : 'No data'); // Debug: cek status pembayaran
        // if (res.data && res.data.status_pembayaran === 'lunas') {
        //     Swal.fire({
        //         icon: 'success',
        //         title: 'Berhasil!',
        //         html: msg + '<br><b>Status: LUNAS</b>',
        //         showCancelButton: true,
        //         confirmButtonText: 'Print Nota',
        //         cancelButtonText: 'Tutup'
        //     }).then((result) => {
        //         if (result.isConfirmed) {
        //             window.open(`/services/${res.data.id}/print`, '_blank');
        //         } else {
        //             window.location.href = "{{ route('services.index') }}";
        //         }
        //     });
        // } else {
        //     // Kalau belum lunas, langsung redirect ke index
        //     Swal.fire({
        //         icon: 'success',
        //         title: 'Berhasil!',
        //         html: msg
                ...
                success: function(res) {
                    if (res.status) {
                        ...
                        Swal.fire({
                            ...
                            // optional: reset select2 / tabel
                            $('.select2').val(null).trigger('change');
                            $('#jobs-table tbody').empty();
                            $('#spareparts-table tbody').empty();
                            $('#total-jobs, #total-spareparts, #grand-total').val('');
                            $('#amount_paid_display, #change_display').val('');
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
...
//     const totalText = $('#grand-total').val().replace(/[Rp\s.]/g, '').replace(',', '.');
//     const total = parseFloat(totalText) || 0;
//     const change = paid - total;
//     $('#change').val(change >= 0 ? new Intl.NumberFormat('id-ID', {
//         style: 'currency',
//         currency: 'IDR',
//         minimumFractionDigits: 0
//     }).format(change) : 'Rp 0');
